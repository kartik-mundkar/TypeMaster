...
class MultiplayerRaceService {
  ...
  setupSocketHandlers() {
    this.io.on('connection', (socket) => {
      console.log(`👤 Player connected: ${socket.id}`);      // Join a race (either find existing or create new)
    ...
    const waitingRaces = await Race.countDocuments({ status: 'waiting' });
    console.log(`📊 Current races: ${totalRaces} total, ${waitingRaces} waiting`);
    
    // Find an available race or create a new one
    let race = await this.findAvailableRace();
    console.log(`🔍 Found available race:`, race ? race.raceId : 'none');
  ...
  async handleJoinRaceById(socket, { raceId, username, isGuest = true, userId = null }) {
    const race = await Race.findOne({ raceId, status: 'waiting' });
    
    if (!race) {
      throw new Error('Race not found or already started');
    }
  ...
  async joinPlayerToRace(socket, race, playerData) {
    ...
    console.log(`Players before adding: ${race.players.length}`);

    await race.addPlayer(player);

    console.log(`Players after adding: ${race.players.length}`);
  ...
  async handleLeaveRace(socket) {
    try {
      ...
      if (race) {
        ...
        // Notify other players
        socket.to(race.raceId).emit('player-left', {
          username: playerInfo.username,
          remainingPlayers: race.players.length
        });        // If race becomes empty, schedule cleanup with a delay to allow other players to join
        if (race.players.length === 0) {
          console.log(`⏳ Race ${race.raceId} is now empty. Scheduling cleanup in 30 seconds...`);
          setTimeout(async () => {
            try {
              // Re-fetch race to check if it's still empty
              const raceCheck = await Race.findOne({ raceId: race.raceId });
              if (raceCheck && raceCheck.players.length === 0 && raceCheck.status === 'waiting') {
                await Race.deleteOne({ _id: raceCheck._id });
                console.log(`🗑️ Deleted empty race: ${race.raceId}`);
              } else if (raceCheck) {
                console.log(`✅ Race ${race.raceId} now has ${raceCheck.players.length} players, keeping it`);
              }
            } catch (error) {
    ...
    // 1. In 'waiting' status (not started, not finished)
    ...
    console.log(`🔍 Available race search result: ${race ? `Found ${race.raceId} with ${race.players.length}/${race.maxPlayers} players` : 'none'}`);
    return race;
  }
